...
static constexpr int    Np = lo * (L * L) * 4 / M_PI;
...
bool ini_hex(double (*x)[dim2]) {
    int NP_NO = lo * 4 * L * L * L * L / (L * L - M_PI * R * R) / M_PI;
...
void output_ani(double (*v)[dim], double (*x)[dim2], double *theta) {
    static int l = 1;
    char       filename[128];
    ofstream   file;
...
bool out_setup() { // filenameが１２８文字を超えていたらfalseを返す;
    ...
    int      test = snprintf(filename, 128,
                             ...
                             folder_name, R, lo, mass, tau, v0, lo, tau, mgn, tmax);
    std::cout << test << endl;
    file.open(filename, std::ios::app); // append
...
void calc_corr(double (*x)[dim2], double (*x0)[dim], double (*v1)[dim],
    ...
    for (int i = 0; i < Np; i++) {
        ...
        msd += ((rsin[0] - x0[i][0] - gc[0]) * (rsin[0] - x0[i][0] - gc[0]) +
                (rsin[1] - x0[i][1] - gc[1]) * (rsin[1] - x0[i][1] - gc[1])) *
               Np_1;
        msd2 = ((rsin[0] - x0[i][0]) * (rsin[0] - x0[i][0]) +
...
void calc_t_dep(double (*x)[dim2], double (*v)[dim], long long j,
    ...
    snprintf(filename, 128,
             "./%sR%.1flo%.2fMs%.3ftau%.3fv0%.1f/"
             "v_s_lo%.3f_tau%.3f_m%.3f.dat",
             folder_name, R, lo, mass, tau, v0, lo, tau, mgn);
    file.open(filename, std::ios::app); // append
    file << j * dt << "\t" << del_vs << endl;
    file.close();
}
...
int main() {
    ...
    ini_hist(theta, Np);
    char foldername[128];
    snprintf(foldername, 128, "%sR%.1flo%.2fMs%.3ftau%.3fv0%.1f", folder_name,
             R, lo, mass, tau, v0);
    ...
    int ituibi = 0, toch = tmax / tmtimes / dt, tanibitch = tbitani / dt;
    ...
    snprintf(filename, 128,
             ...
             folder_name, R, lo, mass, tau, v0, lo, mgn);
