...
bool input(vector<vector<double>> &inpp, char *foldername_1) {
    char filename[128];
    snprintf(filename, 128, "%s/fais_R%.3f.dat", foldername_1, R);
    cout << filename << endl;
    std::ifstream file;
    file.open(filename);
    if (!file) {
...
void output_ft(vector<vector<double>> result, char *folder_name_1) {
    char filename[128];
    snprintf(filename, 128, "%s/fais_qs_R%.3f.dat", folder_name_1, R);
    std::ofstream ofs;
    ofs.open(filename);
    ofs << "#波長　その他;";
    for (auto &&ve : result) {
...
bool do_fts(char *folder_1) {
    ...
    if (inp.empty()||inp.size()<2) return false;
    // sort(inp.begin(), inp.end(),
    //      [](const vector<double> &alpha, const vector<double> &beta) {
    //        return alpha[0] < beta[0];
    //      });
    vector<vector<double>> result(inp.size(), vector<double>(inp[0].size()));
    dft(inp, result);
    cout << "No" << endl;
