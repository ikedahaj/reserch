...
inline void calc_force_wall_gay_bane(double *x, double *f, double theta,
                                     double *f_theta) {
    constexpr double gay_alpha = (gay_kappa * gay_kappa - 1.) /
                                 (gay_kappa * gay_kappa + 1.),
                     gay_alpha_p = (nth_root(gay_kappa_p, gay_M) - 1) /
                                   (nth_root(gay_kappa_p, gay_M) + 1),
                     cutwf2 =
                         (R + 1.5 - cut - 1. / nth_root(1 - gay_alpha, 2)) *
                         (R + 1.5 - cut - 1. / nth_root(1 - gay_alpha, 2)),
                     gay_alpha2 = gay_alpha * gay_alpha,
                     gay_alpha_p2 = gay_alpha_p * gay_alpha_p;
    double r2, sa;
    ...
    if (r2 > cutwf2) {
        ...
        xi = R + 0.5 - r - sigma + 1.;
        if (xi < cut) {
            double sigma3, bun_alpha_p, r_1, r2_1, epsilon13, xi_1, xi2,
                xi6, UWCA, dUWCA, dx_xi, dy_xi, dt_xi, epsilon1, epsilon1_N_1,
            ...
            dy_xi = r_1 * x[1] - x[0] * sigma3 * gay_alpha * (1 - gay_alpha2) *
                                     dr_dot_et * dt_dr_dot_dt * bun_alpha *
                                     bun_alpha;
            dt_xi = gay_alpha * sigma3 * r2*(1-gay_alpha2) *
                    dr_dot_et * dt_dr_dot_dt * bun_alpha * bun_alpha;
            epsilon1 =sqrt(bun_alpha)*r_1;
            epsilon1_N_1 = fast_power(epsilon1, gay_N - 1);
            ...
            dt_epsilon2 = 2 * gay_alpha_p * dr_dot_et * dt_dr_dot_dt *
                        r2*(1-gay_alpha_p2) * bun_alpha_p * bun_alpha_p;
            ...
            f[1] -= (dUWCA * dy_xi * epsilon1 * epsilon2 +
                    ...
                    epsilon1_N_1 * epsilon2_M_1;
            double t1=dUWCA * dt_xi * epsilon1 * epsilon2,t2=UWCA * dt_epsilon1 * gay_N * epsilon2,t3=dt_epsilon2 * epsilon1 * gay_M*UWCA;
...
/// @param theta 粒子の角度.値私で良い;
/// @param f_theta 確度にかかる力.(&f[i])などと渡すこと.
inline void calc_wall_gay_bane_anill(double *x, double *f, double theta,
    ...
    double r2, sa,
                     cutwf2 =
                         (R_ini + 1.5 - cut - 1. / nth_root(1 - gay_alpha, 2)) *
                         (R_ini + 1.5 - cut - 1. / nth_root(1 - gay_alpha, 2));
    if (x[0] > 0)
        sa = R_ini*rbit_2;
    else
...
void cell_list(int (*list)[Nn], double (*x)[dim]) {
    ...
    constexpr int    My =
        (int) (2. * R / (cut + gay_kappa - 1 + skin));  // M<=2R/(cutmax+skin)
    ...
    for (int i = 0; i < Np; ++i) {
        ...
        for (int m = max(nx[1] - 1, 0), mm = min(nx[1] + 1, My - 1); m <= mm;
             ++m) {
