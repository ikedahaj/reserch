...
#define v0          1.
#define tau         50. // コンパイル時に-D{変数名}={値}　例:-Dtau=80　とすること;
...
#define dtlg        0.0001
#define dt          0.0001
#define folder_name "stwkanR20_5_M80"
#define msdbit      1.2
#define msdini      0.01
...
// delg の感覚で動くlangevin方程式;
void eom_langevin_s(double (*v)[dim], double (*x)[dim], double (*f)[dim],
                    int (*list)[Nn]) {
    double fiw[dim], sinco[2], fluc = sqrt(2 * dtlg);
    calc_force(x, f, list);
    for (int i = 0; i < Np; i++) {
        calc_force_2w(x[i], fiw);
        for (int j = 0; j < dim; j++) {
...
void eom_abp9(double (*v)[dim], double (*x)[dim], double (*f)[dim],
    ...
    constexpr double ddt = 1e-7, D = usr_sqrt(2. * ddt / tau),M_inv=ddt/mass;
    calc_force(x, f, list);
    for (int i = 0; i < Np; i++) {
        calc_force_2w(x[i], fiw);
        ...
        v[i][0] += (sico[0] + f[i][0] + fiw[0])*M_inv;
        v[i][1] += (sico[1] + f[i][1] + fiw[1])*M_inv;
        x[i][0] += v[i][0] * ddt;
        x[i][1] += v[i][1] * ddt;
    }
}
...
void eom_abp1(double (*v)[dim], double (*x)[dim], double (*f)[dim],
    ...
    constexpr double M_inv=dt/mass;
    calc_force(x, f, list);
    for (int i = 0; i < Np; i++) {
        calc_force_2w(x[i], fiw);
        theta_i[i] += D * gaussian_rand() + Mg;
...
void make_v_thetahist(double (*x)[dim], double (*v)[dim], double(*hist),
    ...
    double              v_t, dr;
    static const double bunbo = 1. / (tmax / tau);
    int                 histint;
    for (int i = 0; i < Np; ++i) {
...
void output(double (*v)[dim], double (*x)[dim]) {
    ...
    ofstream   file;
    sprintf(filename,
            "./%sR%.1frs%.1f_coorlo%.2ftau%.3fv0%.1f/"
            "tyouwaenn_m%.3f_t%d.dat",
            folder_name,R,R_in, lo, tau, v0,mgn ,  l);
...
void output_ani_ini(double (*v)[dim], double (*x)[dim]) {
    ...
    ofstream file;
    sprintf(filename,
            "./%sR%.1frs%.1f_animelo%.2ftau%.3fv0%.1f/"
            "tyouwaenn_m%.3f_t%d.dat",
            folder_name,R,R_in, lo, tau,  v0,mgn,  0);
    ...
    sprintf(filename,
            "./%sR%.1frs%.1f_animelo%.2ftau%.3fv0%.1f/"
            "tyokkei_m%.3f.dat",
            folder_name,R,R_in, lo, tau,  v0,mgn);
    file.open(filename /* std::ios::app*/); // append
    file << tbitani << endl;
    for (int i = 0; i < Np; ++i) {
        file << 1 << endl;
...
void out_setup() {
    ...
    sprintf(filename, "./%sR%.1frs%.1flo%.2ftau%.3fv0%.1f/setupr%fm%f.dat",
            folder_name,R,R_in, lo, tau,  v0, R, mgn);
    file.open(filename, std::ios::app); // append
...
void outputhist(double *hist, int counthistv_theta, double *lohist,
    ...
    sprintf(filename,
            ...
            folder_name,R,R_in, lo, tau,  v0, lo, tau, mgn);
    file.open(filename /*,std::ios::app*/); // append
    ...
    sprintf(filename,
            ...
            folder_name,R,R_in, lo, tau,  v0, lo, tau);
    file.open(filename, std::ios::app); // append
...
int main() {
    ...
    char foldername[128];
    sprintf(foldername, "%sR%.1frs%.1flo%.2ftau%.3fv0%.1f", folder_name, R,R_in,lo,
            tau, mgn, v0);
    const char *fname = foldername;
    mkdir(fname, 0777);
    char foldername2[128];
    sprintf(foldername2, "%sR%.1frs%.1f_coorlo%.2ftau%.3fv0%.1f",
            folder_name,R,R_in, lo, tau,  v0);
    const char *fname2 = foldername2;
    mkdir(fname2, 0777);
    sprintf(foldername, "%sR%.1frs%.1f_animelo%.2ftau%.3fv0%.1f",
            folder_name,R,R_in, lo, tau,  v0);
    ...
    sprintf(filename, "./%sR%.1frs%.1flo%.2ftau%.3fv0%.1f/kekkalo%.3fm%.3f.dat",
            folder_name,R,R_in, lo, tau,  v0, lo, mgn);
    file.open(filename, std::ios::app); // append
    ...
    snprintf(filename, 128,
             "%sR%.1frs%.1f_animelo%.2ftau%.3fv0%.1f/tyokkei.dat",
             folder_name,R,R_in, lo, tau,  v0);
    file.open(filename);
    file << tbitani << endl;
    for (int i = 0; i < Np; i++) {
