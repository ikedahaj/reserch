...
#define Np     3 // 8192// 4096// 4の倍数であること;
#define Nn     3
#define R   22.36067977  //50.//28.86751346//  32.//64.//128.// lo=0.5,N1000//3.
                //  //R=sqrt(np/4/lo);
// ,0.1より大きいこと;
#define tmax     500 // 973.686//2*100たうとする;
...
#define mgn         50.// Omega=omega/tau,ここではomegaを入れること;
...
void eom_aoup(double (*v)[dim], double (*x)[dim], double (*f)[dim], double *a,
              double temp0, int (*list)[Nn], double (*F)[dim],double *count) {
    double tauinv = dt / tau;
    double F0[dim],v0[2];
    double fluc = sqrt(2.*temp0 / dt), dr,
           cou = 1. / (tmax * ensemble);
    ...
    for (int i = 0; i < Np; i++) {
        ...
        x[i][0] += v0[0] * dt;
        x[i][1] += v0[1] * dt;
        dr = x[i][0] * x[i][0] + x[i][1] * x[i][1];
        /////////////*wall boundaly
        if (dr > (R-a[i])*(R-a[i])) {
            count[i] += cou;
            double ddr = (R-a[i]) / sqrt(dr);
            x[i][0] *= ddr;
            x[i][1] *= ddr;

            v[i][0] = x[i][1] * (v0[0] * x[i][1] - v0[1] * x[i][0]) / dr;
            v[i][1] = x[i][0] * (v0[1] * x[i][0] - v0[0] * x[i][1]) / dr;
        } else {
            v[i][0] = v0[0];
            v[i][1] = v0[1];
        }
...
int main() {
    ...
    while (j * dtlg < 10) {
        ...
        eom_aoup(v, x, f, a, ttemp, list, F,countbound);
    ...
    while (j * dt < tmaxlg) {
        j++;
        auto_list_update(&disp_max, x, x_update, list);
        eom_aoup(v, x, f, a, temp, list, F,countbound);
    }
    ...
    std::ofstream file;
    double        sum = 0.;
    sprintf(filename, "./%slo%.2fv0%.1ftau%.3fm%.3f/boundlo%.3fm%.3f.dat",
            folder_name, Np * 0.25 / (R * R), temp / tau, tau, Mgn,
            Np * 0.25 / (R * R), Mgn);
    file.open(filename, std::ios::app); // append
    for (int i = 0; i < Np; i++) {
    ...
    sprintf(filename, "./%slo%.2fv0%.1ftau%.3fm%.3f/boundcuntlo%.3fm%.3f.dat",
            folder_name, Np * 0.25 / (R * R), temp / tau, tau, Mgn,
            Np * 0.25 / (R * R), Mgn);
    file.open(filename, std::ios::app); // append
