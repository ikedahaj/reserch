...
static constexpr double R =6.; // 13.07252733; // 5.; // 固定;// ,0.1より大きいこと;
////parameters
constexpr double
...
double calc_force(double (*x)[dim], double (*f)[dim], int (*list)[Nn]) {
    double dx, dy, dr2, dUr, w2, w6 /*,  aij /*w12*/,U=0.;
    ini_array(f);
    for (int i = 0; i < Np; ++i)
...
bool out_setup() { // filenameが１２８文字を超えていたらfalseを返す;
    ...
    int      test = snprintf(filename, 128,
                             ...
                             folder_name, mass, tau, Rbit, v0, lo, tau, mgn, tmax);
    std::cout << test << endl;
    file.open(filename, std::ios::app); // append
...
void output_ene(double (*v)[dim], double U,int j) {
    double K = 0.;
    for (int i = 0; i < Np; i++) {
        K += 0.5*(v[i][0] * v[i][0] + v[i][1] * v[i][1]) / Np;
    }
    char     filename[128];
    ofstream file;
    snprintf(filename, 128, "./%slo%.2fMs%.3ftau%.3fbit%.3fv0%.1f/ene_fstR%.3f",
             folder_name, lo, mass, tau, Rbit, v0, R);
    file.open(filename,std::ios::app);
    file<<j*dt<<" "<<K<<" "<<U/Np<<" "<<K+(U)/Np<<endl;
    file.close();
}
int main() {
    ...
    j = 0;
    double outflag_c = 0.01, outflag_en = 0.,U=0.;
    while (j < 1e7) {
        ++j;
        auto_list_update(&disp_max, x, x_update, list);
    ...
    while (j < tmaxbefch) {
        ++j;
        auto_list_update(&disp_max, x, x_update, list);
        eom_abp1(v, x, f, list, theta,&U);
        if (j * dt >= outflag_c) {
            calc_corr(x, x0, v1, v, msd, vcor, msd2);
            outflag_c *= 1.2;
        }
        if(j*dt>=outflag_en){
            output_ene(v,U,j);
    ...
    ofstream file2;
    snprintf(filename, 128,
             "./%slo%.2fMs%.3ftau%.3fbit%.3fv0%.1f/kekkaR%.3fm%.3f.dat",
             folder_name, lo, mass, tau, Rbit, v0, R, mgn);
