...
#define mgn         0.05  // Omega=omega/tau,ここではomegaを入れること;
...
static constexpr double Mgp = mgn * dt;
static constexpr double Mgm=-mgn*dt;
...
void eom_abp8(double (*v)[dim], double (*x)[dim], double (*f)[dim], double *a,
    ...
    for (int i = 0; i < Np; i++) {
        ...
        // till here*/
        if(i%2==0)theta_i[i] += D * gaussian_rand() + Mgp;
        else theta_i[i] += D * gaussian_rand() + Mgm;
        theta_i[i] -= (int) (theta_i[i] * M_1_PI) * M_PI2;
...
void outputhist(double *hist, int counthistv_theta, double *lohist,
    ...
    snprintf(
        filename, 128,
        "./%sR%.1flo%.2ftau%.3fm%.3fbit%.3fv0%.1f/v_theta_lo%.3f_tau%.3f.dat",
        folder_name, R, lo, tau, mgn, Rbit, v0, lo, tau);
    file.open(filename, std::ios::app); // append
...
void outputcorr(double *msd, double *vcor, double *t, int countout,
    ...
    snprintf(
        filename, 128,
        "./%sR%.1flo%.2ftau%.3fm%.3fbit%.3fv0%.1f/msd_lo%.3f_tau%.3f_m%.3f.dat",
        folder_name, R, lo, tau, mgn, Rbit, v0, lo, tau, mgn);
    file.open(filename /*,std::ios::app*/); // append
    for (int i = 0; i < countout; ++i) {
