...
// usr_sincos(theta_i[i], sico);
    ...
    for (int i = Np_act; i < Np; i++) {
        calc_force_wall(x[i], f[i]);
        v[i][0] += (-v[i][0] + f[i][0]) * ddt + D_l * gaussian_rand();
        v[i][1] += (-v[i][1] + f[i][1]) * ddt + D_l * gaussian_rand();
        x[i][0] += v[i][0] * ddt;
        x[i][1] += v[i][1] * ddt;
    }
}
...
void eom_abp1(double (*v)[dim], double (*x)[dim], double (*f)[dim],
    ...
    static const double     D = sqrt(2. * dt / tau), D_l = sqrt(0.02 * dt);
    calc_force(x, f, list);
    for (int i = 0; i < Np_act; i++) {
        calc_force_wall(x[i], f[i]);
        theta_i[i] += D * gaussian_rand() + Mg;
...
// usr_sincos(theta_i[i], sico);
    ...
    for (int i = Np_act; i < Np; i++) {
        calc_force_wall(x[i], f[i]);
        v[i][0] += (-v[i][0] + f[i][0]) * dt + D_l * gaussian_rand();
        v[i][1] += (-v[i][1] + v[i][1]) * dt + D_l * gaussian_rand();
        x[i][0] += v[i][0] + dt;
        x[i][1] += x[i][1] * dt;
    }
}
