...
static constexpr double R =
    4.5 + cut - 0.5; // 13.07252733;  // 固定;// ,0.1より大きいこと;
////parameters
...
void eom_langevin_h(double (*v)[dim], double (*x)[dim], double (*f)[dim],
    ...
    double zeta = 1.0;
    double fluc = sqrt(2. * zeta * 5. * dtlg), const_fst = -48.;
    double ri, riw, w2, w6, dUr;
    calc_force(x, f, list);
    for (int i = 0; i < Np; i++) {
...
void eom_langevin_l(double (*v)[dim], double (*x)[dim], double (*f)[dim],
    ...
    double fluc = sqrt(2. * temp * dtlg), const_fst = -48.;
    double ri, riw, w2, w6, dUr;
    calc_force(x, f, list);
    for (int i = 0; i < Np; i++) {
