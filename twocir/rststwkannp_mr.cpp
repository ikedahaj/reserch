...
#define Nn 50
#define tmax 250  // 973.686//2*100たうとする;<tmaxaniの時気をつける;
#define tmaxlg 100   // 緩和時間は10たうとする;
#define tmaxani 250  //>tmaxの時プログラムを変更すること;
#define tbitani 1
...
#define w_force calc_force  // calc_force:WCA;
...
constexpr double usr_arccos(double theta) {
  ...
  double asi =
      (((((waru[0] * x + waru[1]) * x + waru[2]) * x + waru[3]) * x + waru[4]) *
           x +
       1.) *
      theta;
  return M_PI_2 - asi;
...
void usr_sincos(double kaku, double *x) {  // x[0]がcos,x[1]issin;
  ...
  s = ((((waru[4] * z + waru[5]) * z + waru[6]) * z + waru[7]) * z + 1.) * kaku;
...
void eom_langevin(double (*v)[dim], double (*x)[dim], double (*f)[dim],
  ...
  for (int i = 0; i < Np; i++) {
    ...
    w_wall(x[i], f[i]);
    for (int j = 0; j < dim; j++) {
      v[i][j] += -zeta * v[i][j] * ddt + f[i][j] * ddt + fluc * gaussian_rand();
      x[i][j] += v[i][j] * ddt;
    }
  }
}
void eom_langevin_h(double (*v)[dim], double (*x)[dim], double (*f)[dim],
  ...
  for (int i = 0; i < Np; i++) {
    ...
    w_wall(x[i], f[i]);
    for (int j = 0; j < dim; j++) {
      v[i][j] += -v[i][j] * dtlg + f[i][j] * dtlg + fluc * gaussian_rand();
      x[i][j] += v[i][j] * dtlg;
    }
  }
}
...
void output_ini(double (*v)[dim], double (*x)[dim]) {
  ...
  file.close();
  snprintf(filename, 128, "./%s/coor_R%.3f_m%.3f_t0.dat", foldername1, R, mgn);
...
void output(double (*v)[dim], double (*x)[dim]) {
  ...
  snprintf(filename, 128, "./%s/coor_R%.3f_m%.3f_t%d.dat", foldername1, R, mgn,
           l);
  file.open(filename);  // append
  for (int i = 0; i < Np; ++i) {
    file << x[i][0] << "\t" << x[i][1] << "\t" << v[i][0] << "\t" << v[i][1]
...
void cell_list(int (*list)[Nn], double (*x)[dim]) {
  ...
  for (int i = 0; i < Np; ++i) {
    ...
    for (int m = usr_max(nx[i][1] - 1, 0), mm = usr_min(nx[i][1] + 1, My - 1);
         m <= mm; ++m) {
      for (int l = usr_max(nx[i][0] - 1, 0), lm = usr_min(nx[i][0] + 1, Mx - 1);
           l <= lm; ++l) {
        map_index = l + Mx * m;
...
int main() {
  ...
  file2 << R << " " << faimax << " " << c_fai << " " << pibarmax << " " << lzmax
        << " " << ommax << endl;
  file2.close();
