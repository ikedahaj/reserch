...
#define Nn                    500
#define tmax                  10  // 973.686//2*100たうとする;<tmaxaniの時気をつける;
#define tmaxlg                000  // 緩和時間は10たうとする;
#define tmaxani               10  //>tmaxの時プログラムを変更すること;
#define tbitani               0.1
#define dim                   2  // 変えるときはEomを変えること;
#define skin                  1.5
#define dtlg                  1e-5
#define dt                    1e-5
#define folder_name           "lange"  //"h0" // 40文字程度で大きすぎ;
...
#define dist_pwall            0.2  // 壁を構成する粒子同士の距離;
...
#endif
#define Rbit 0.  // delta/R,Rにすると穴がなくなる;0のみ//
...
static constexpr int Np_wall = M_PI2 * (R + 0.5) * (R + 0.5) / dist_pwall;
...
bool ini_coord_twocircles(double (*x)[dim]) {
    double R2 = R - (0.2), rbbit = Rbit * 0.5,
           bit = sqrt((R2 * R2 *
    ...
    for (int i = 0; i <= nmax; i++) {
        for (int j = 0; j <= nmax; j++) {
            double r2[2] = {i * bit + 0.5, j * bit + 0.5};
...
void ini_coord_wall(double (*x)[dim]) {
    double rat_theta = dist_pwall / ((R + 0.5) * (R + 0.5));
    for (int i = Np; i < Np + Np_wall; i++) {
        x[i][0] = (R + 0.5) * cos((i - Np) * rat_theta);
        x[i][1] = (R + 0.5) * sin((i - Np) * rat_theta);
    }
}
...
void calc_force_harmonic(double (*x)[dim], double (*f)[dim], int (*list)[Nn]) {
    ...
    constexpr double c_f = (ratf > 10) ? 2. * ratf : 2 * 50;
    ini_array(f);
    for (int i = 0; i < Np; ++i)
        for (int j = 1; j <= list[i][0]; ++j) {
            dx = x[i][0] - x[list[i][j]][0];
...
void eom_langevin(double (*v)[dim], double (*x)[dim], double (*f)[dim],
    ...
    double fluc = sqrt(2. * zeta * 0.01 * ddt);
    w_force(x, f, list);
    for (int i = 0; i < Np; i++) {
        for (int j = 0; j < dim; j++) {
            v[i][j] +=
...
void          cell_list(int (*list)[Nn], double (*x)[dim]) {
    ...
    static constexpr int Mx = (int) (xlen_2 * 2. / (cut + skin)) + 2;
    static constexpr int My =
        (int) (2. * R / (cut + skin)) + 2;  // M<=2R/(cutmax+skin)
    static constexpr int    m2 = Mx * My;
    static constexpr double R2 = 2. * R, bitx = (Mx - 2) / (xlen_2 * 2.),
                            bity = (My - 2) / (R2), tasu_x = xlen_2 + 1. / bitx,
                            tasu_y = R + 1. / bity;  // ひとつのせるの幅の逆数;
    double dx, dy;
    // int(*map)[Np + 1] = new int[m2][Np + 1];
    std::vector<std::deque<int>> map(m2);

    for (int i = 0; i < Np + Np_wall; ++i) {
        nx[0] = (int) ((x[i][0] + tasu_x) * bitx);
        nx[1] = (int) ((x[i][1] + tasu_y) * bity);
        for (int m = usr_max(nx[1] - 1, 0), mm = usr_min(nx[1] + 1, My - 1);
             m <= mm; ++m) {
            for (int l = usr_max(nx[0] - 1, 0), lm = usr_min(nx[0] + 1, Mx - 1);
                 l <= lm; ++l) {
                map_index = l + Mx * m;
    ...
    for (int i = 0; i < Np; ++i) {
        list[i][0] = 0;
        map_index = (int) ((x[i][0] + tasu_x) * bitx) +
                    Mx * (int) ((x[i][1] + tasu_y) * bity);
...
int main() {
    ...
    double v[Np][dim], theta[Np], f[Np][dim], x0[Np][dim], v1[Np][dim],
        x_update[Np][dim];
    double x[Np + Np_wall][dim];
    // double(*x)[2] = new double[Np + Np_wall][2];
    int    list[Np][Nn];
    int    countout = 0;
    double tout = msdini;
    // set_diameter(a);
