...
#define Nn             50
#define tmax           10000 // 973.686//2*100たうとする;<tmaxaniの時気をつける;
...
void eom_abp9(double (*v)[dim], double (*x)[dim], double (*f)[dim], double *a,
    ...
    static constexpr double fluc = usr_sqrt(2. * zeta * ddt), D = usr_sqrt(2. * ddt / tau);
...
void eom_langevin(double (*v)[dim], double (*x)[dim], double (*f)[dim],
    ...
    double zeta = 1.0;
    static double fluc = sqrt(temmp * zeta * dtlg), D = sqrt(2. * dtlg / tau), M_inv = dt / mass;
    double vi[2], ri, riw, aij, w2, w6, dUr, fiw[dim];
    calc_force(x, f, a, list);
    for (int i = 0; i < Np; i++) {
        fiw[0] = 0.;
